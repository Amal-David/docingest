...
function providerTotals(data: any, returned: number): CrawlProviderTotals {
  return {
    discovered: getCrawlTotal(data, returned),
    returned,
...
export async function getCrawlStatus(crawlId: string): Promise<CrawlStatusResponse> {
  if (!isCloudflareConfigured()) {
    return {
      status: 'failed', completed: 0, total: 0, data: [], outcomes: [],
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 429) {
        ...
        return {
          status: 'scraping', completed: 0, total: 0, data: [], outcomes: [],
          providerTotals: { discovered: 0, returned: 0, discoveredIsExact: false },
        };
      }
    ...
    if (normalizedStatus !== 'completed') {
      return {
        ...
        data: [],
        outcomes: [],
        providerTotals: providerTotals(data, rawResults.length),
      };
    }
    ...
    for (const record of rawResults) {
      ...
      if (normalizedUrl && seenUrls.has(normalizedUrl)) {
        outcomes.push({ url: pageUrl, canonicalUrl: normalizedUrl, status: 'duplicate', reason: 'duplicate-provider-url' });
        continue;
      }
      ...
      if (markdown) {
        ...
        outcomes.push({ url: pageUrl, canonicalUrl: normalizedUrl || pageUrl, status: 'valid' });
      } else {
        outcomes.push({ url: pageUrl, canonicalUrl: normalizedUrl || pageUrl, status: 'empty', reason: 'provider-result-had-no-extractable-content' });
      }
    }
